...
<template>
  <div class="rounded-lg border border-slate-200 bg-white p-5">
    ...
    <div class="mt-4 border-t border-slate-100 pt-4">
      <span class="block rounded-md border px-3 py-1.5 text-center text-xs font-medium" :class="outcomeClasses">
        Outcome: {{ outcome }}
      </span>
    </div>
    ...
    <button
      type="button"
      class="mt-3 flex w-full items-center justify-center gap-1.5 rounded-md px-4 py-2 text-sm font-medium transition disabled:cursor-not-allowed"
      :class="
        isLocked
    ...
    <button
      type="button"
      class="mt-2 flex w-full items-center justify-center gap-1 rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50"
