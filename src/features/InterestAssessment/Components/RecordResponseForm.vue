...
function handleSubmit() {
  ...
  const finalCandidateId = candidateIdInput.value || selectedCandidateId.value
  emit('submit', { candidateId: finalCandidateId, answers: questionAnswers })
}
</script>
...
<template>
  <div class="rounded border border-slate-200 bg-white p-5">
    <div class="grid grid-cols-2 gap-4">
      <div class="block">
        <span class="font-mono text-[11px] font-medium uppercase tracking-wider text-slate-400">Candidate name</span>
        <BaseSelect
      ...
      <div class="block">
        <span class="font-mono text-[11px] font-medium uppercase tracking-wider text-slate-400">Candidate ID</span>
        <BaseInput
          v-model="candidateIdInput"
          placeholder="Type candidate ID"
          class="mt-1.5"
        />
      </div>
    </div>
    ...
    <div class="mt-6 flex justify-end">
      <BaseButton
        class="!w-auto"
        :disabled="!selectedCandidateId && !candidateIdInput"
        @click="handleSubmit"
      >
        Submit response
      </BaseButton>
